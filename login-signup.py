...
file_exists=os.path.exists("/home/sabrina/Desktop/login.json")
def dump(obj):
    with open("/home/sabrina/Desktop/login.json","w") as f:
...
if user=="signup":
    ...
    if re.search ("[a-z A-Z]",password1) and re.search ("[@%$#!]",password1) and re.search ("[0-9]",password1) :
        ...
        if password1==password2:
            ...
            if file_exists==True:
                with open("/home/sabrina/Desktop/login.json","r") as file:
                    d=file.read()
                    p=json.loads(d)
                    p.append(dic)
                    dump(p)
